...
void matchStr(char* str){
	...
	if(Trie->firstChild==NULL){
		...
		struct node* ptr = Trie->firstChild;
		while(ptr!=NULL){
			for(count=ptr->sIndex;count<ptr->eIndex+1&&count<strlen(str);count++){//check letter one by one
				if(str[count]!=(ptr->word)[count]){
					if(ptr->sibling!=NULL){
						ptr=ptr->sibling;
						count=ptr->sIndex-1;//becuase next turn count would +1
						continue;
					}
					else{
						return;
			...
			if(count==ptr->eIndex+1&&count==strlen(str)){//have been to the end
				if(ptr->isWord==1){
					ptr->Occurrence++;
				}
				return;
			}
			else if(count==ptr->eIndex+1&&count<strlen(str)){
				if(ptr->isWord==1){
					ptr->superWord++;
				}
				ptr=ptr->firstChild;
			}
			else{//count<ptr->eIndex&&count==strlen(str)
				return;
			}
...
void traverse(FILE *file_name, struct node* Node){
	...
	if(Node==NULL){
		...
		ptr= Node;
			if(ptr->isWord==1){
				fprintf(file_name,"%s %d %d\n",ptr->word,ptr->Occurrence, ptr->superWord);
